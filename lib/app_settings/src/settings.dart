...
final class Preferences {
  ...
  final fetchVideoInfosCmd =
      'yt-dlp --verbose --simulate --no-flat-playlist --no-mark-watched --print "%(.{title,id,description,uploader,upload_date})j" --restrict-filenames --windows-filenames --retries 2147483647 --fragment-retries 2147483647 --extractor-retries 0 --cookies "<cookie_file>" "https://www.youtube.com/playlist?list=<playlist_id>"';
  ...
  final fetchPlaylistItemCountCmd =
      'yt-dlp --verbose --playlist-items 0-1 --simulate --no-flat-playlist --no-mark-watched --print "%(.{playlist_count})j" --retries 2147483647 --fragment-retries 2147483647 --extractor-retries 0 --cookies "<cookie_file>" "https://www.youtube.com/playlist?list=<playlist_id>"';
  ...
  final videoPreferredCmd =
      'yt-dlp --verbose --paths "<output_dir>" --format "bestvideo[width<=1920][height<=1080][fps<=60][vcodec^=av01]+bestaudio[acodec=opus][audio_channels<=2][asr<=48000]" --output "%(title)s" --restrict-filenames --merge-output-format mkv --embed-thumbnail --convert-thumbnail png --embed-metadata --sub-lang "en.*|id.*" --write-subs --progress-template \"{\'percentage\':\'%(progress._percent_str)s\',\'bytes_downloaded\':\'%(progress._downloaded_bytes_str)s\',\'bytes_total\':\'%(progress._total_bytes_str)s\',\'download_speed\':\'%(progress._speed_str)s\',\'ETA\':\'%(progress._eta_str)s\'}\" --fragment-retries 2147483647 --retries 2147483647 --extractor-retries 0 "https://www.youtube-nocookie.com/embed/<video_id>"'; // ommited --cookies "<cookie_file>"
  final videoRegularCmd =
      'yt-dlp --verbose --paths "<output_dir>" --format "bestvideo[width<=1920][height<=1080][fps<=60]+bestaudio[audio_channels<=2][asr<=48000]" --output "%(title)s" --restrict-filenames --merge-output-format mkv --embed-thumbnail --convert-thumbnail png --embed-metadata --sub-lang "en.*|id.*" --write-subs --progress-template \"{\'percentage\':\'%(progress._percent_str)s\',\'bytes_downloaded\':\'%(progress._downloaded_bytes_str)s\',\'bytes_total\':\'%(progress._total_bytes_str)s\',\'download_speed\':\'%(progress._speed_str)s\',\'ETA\':\'%(progress._eta_str)s\'}\" --fragment-retries 2147483647 --retries 2147483647 --extractor-retries 0 "https://www.youtube-nocookie.com/embed/<video_id>"'; // ommited --cookies "<cookie_file>"
